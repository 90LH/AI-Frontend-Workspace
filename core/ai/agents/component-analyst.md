...
1. 理解组件需求、项目范围和分析目标
2. 优先使用 GitNexus 分析组件目录、引用关系、调用链、影响范围和相似组件
3. GitNexus 不可用时，说明降级原因，并使用 `rg`、文件读取和 imports 分析
4. 执行 `core/ai/skills/component-analysis.md`
5. 按组件位置和职责分类：通用组件、业务组件、页面私有组件、第三方封装组件
6. 提取 Props、Emits、Slots、Expose 的接口摘要
7. 分析依赖关系、引用关系、影响范围和潜在复用边界
8. 识别相似组件和重复实现风险
9. 输出复用、扩展、包装、新建、拆分建议
10. 将组件资产报告交给 Architect；需要实现时再交给 Developer；需要审查时交给 Reviewer
