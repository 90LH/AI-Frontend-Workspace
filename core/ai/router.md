...
真实项目级知识必须写入目标项目 `.veaw/`（含 `.veaw/component-catalog/`），不得写入 VEAW `core/`、`presets/` 或 `extensions/`。
...
Component Catalog 位于目标项目的 `.veaw/component-catalog/` 目录下，由 `project-onboarding` 初始化。

使用规则：

- 组件分析任务前，优先读取 `.veaw/component-catalog/catalog.json`（不存在时降级到 `.veaw/component-catalog/index.md`）了解已有资产
...
| L1（按需读） | `.veaw/component-catalog/catalog.json`（如存在） | 组件相关 Skill 开始时 |
...
| `catalog.json` | 读 `.veaw/component-catalog/index.md`；若也不存在，从源码目录扫描 |
...
`.veaw/commands/README.md` 是 Command 注册表的唯一真源（SSOT）。

本节仅负责路由与分发：识别用户触发的 Command 名称后，跳转到 `.veaw/commands/README.md` 的 Command Router 表，按其中登记的 Skills / Agent 链继续执行。

规则：

1. 本文件不重复维护 Command 列表，不在此处新增或复制 Command 表格
2. 新增或修改 Command 时，只更新 `.veaw/commands/README.md` 与对应的 `.veaw/commands/<name>.md`
3. 本文件与 `.veaw/commands/README.md` 冲突时，以 `.veaw/commands/README.md` 为准
